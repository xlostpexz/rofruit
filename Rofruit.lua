...
local DinoWindow = Dino:CreateWindow("Ro Fruit")
...
local Home = DinoPage:NewSection("Auto Farm")
    ...
    spawn(function()
       ...
       game:GetService("RunService").RenderStepped:Connect(function()
        pcall(function()
            if _G.Farm then
    game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame = game:GetService("Workspace").Mob["Angel MLG"].HumanoidRootPart.CFrame * CFrame.new(0,0,3)
game:GetService("ReplicatedStorage").Combat.Combat:FireServer()
    if game:GetService("Workspace").Mob["Angel MLG"].Humanoid.Health <= 2499 then
        game:GetService("Workspace").Mob["Angel MLG"].Humanoid.Health = die
    end
            end
        end)
       end)
